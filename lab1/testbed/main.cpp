...
veekay::mat4 Camera::view_projection(float aspect_ratio) const {
	...
	if (use_orthographic) {
		...
		auto projection = veekay::mat4::orthographic(
...
void initialize(VkCommandBuffer cmd) {
	...
	// NOTE: Octahedron mesh initialization
	{
		...
		octahedron_mesh.index_buffer = new veekay::graphics::Buffer(
...
void update(double time) {
	...
	ImGui::SliderFloat("Rotation speed (rad/s)", &rotation_speed, 0.0f, 6.0f);
	ImGui::Checkbox("Pause animation", &animation_paused);
	ImGui::Checkbox("Reverse direction", &animation_reverse);
	ImGui::Separator();
	ImGui::Checkbox("Orthographic projection", &camera.use_orthographic);
	if (camera.use_orthographic) {
		ImGui::SliderFloat("Ortho height", &camera.ortho_height, 1.0f, 20.0f);
	} else {
	...
	float direction = animation_reverse ? -1.0f : 1.0f;
	animation_time += static_cast<float>(dt) * direction;
